...
export default function Modal({ user }: { user: User }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const { data: dataProfile, error: errorProfile } = await supabase
                .from('profiles')
                .select('estado, id, full_name, alta')
                .eq('user_id', user.id)
            ...
            if (dataProfile && dataProfile.length > 0) {
                ...
                setAlta(dataProfile[0].alta)
                console.log(dataProfile[0].alta)
            }
        }
    ...
    async function fichar() {
        ...
        if (alta) {
            ...
            //Cambiar por una modal o algo
            alert('No estas dado de alta, consulta a un administrador');
        }
    }
    ...
    return (
        ...
        isOpen ? (
            alta && (estado === 'Inactivo' || estado === 'Jornada Finalizada') ? (
